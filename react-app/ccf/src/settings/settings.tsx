...
function AccountSettingsPage(): JSX.Element {
    ...
    const [currentPasswordMatched, setCurrentPasswordMatched] = useState(false);
        ...
        const fetchUserData = async () => {
            if (auth.currentUser?.uid) {
                try {
                    ...
                    setInstitution(user?.institutionalAffiliation || "institution not found");
                } catch (error) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        e.preventDefault();
        console.log(specialChar, capitalLetter, number, showReqs, pwdUnmatched, currentPasswordMatched);
                ...
                <div className="AccountSettings-sections-content">
                        ...
                        <div className="AccountSetting-personal-info">
                            <div className="AccountSetting-personal-info-field">
                                ...
                                <div className="info-row">
                                    <input type="text" value={firstName} onChange={(e) => editApplicantUser(auth.currentUser?.uid || "", {name: e.target.value + " " + lastName})} />
                            ...
                            <div className="AccountSetting-personal-info-field">
                                ...
                                <div className="info-row">
                                    <input type="text" value={lastName} onChange={(e) => editApplicantUser(auth.currentUser?.uid || "", {name: firstName + " " + e.target.value})} />
                                    <span className="edit-icon">✎</span>
                                </div>
                            </div>
                            <div className="AccountSetting-personal-info-field">
                                <label>Title</label>
                                <div className="info-row">
                                    <input type="text" value={title} onChange={(e) => editApplicantUser(auth.currentUser?.uid || "", {title: e.target.value})} />
                                    <span className="edit-icon">✎</span>
                                </div>
                            </div>
                            <div className="AccountSetting-personal-info-field">
                                ...
                                <div className="info-row">
                                    <input type="text" value={institution} onChange={(e) => editApplicantUser(auth.currentUser?.uid || "", {institutionalAffiliation: e.target.value})} />
                                    <span className="edit-icon">✎</span>
                                </div>
                            </div>
                        </div>      
                    ...
                    <div className="AccountSettings-section">
                        ...
                        <div className="info-row-settings">
                            ...
                            <span>{username ? username : "No username available"}</span>
                        </div>
                        <div className="info-row-settings">
                            ...
                            <input
                                ...
                                onChange={async (e) => {
                                    ...
                                    const { error: loginError } = await loginUser(auth.currentUser?.email || "", currentPassword);
